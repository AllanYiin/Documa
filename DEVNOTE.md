...
### 需求狀態
- [x] 2026-09-03：新增手動觸發的 `Linux wheels` GitHub Actions workflow；Ubuntu 24.04 runner 透過 manylinux_2_28 建置 CPython 3.10／3.11／3.12 x86_64 wheel，在隔離安裝環境執行既有 release MCP smoke 與 pip check，成功後上傳 wheel／SHA256SUMS。YAML 與本機 smoke 路徑檢查 PASS；目前原始碼全套 443 passed／6 skipped、Ruff／plugin validator／deterministic ZIP／LingXi 模型雜湊 PASS。尚未實際執行 Linux wheel job，不能宣稱 Linux artifact 已驗證；release smoke 目前固定檢查 Documa 0.8.0／LingXi 0.4.5，升版時須同步。
...
- `package_plugins.py` 現在對已知 UTF-8 text suffix 在寫入 ZIP 前統一 CRLF／CR 為 LF；PNG 等 binary payload 不變。ZIP entries 改用 `ZIP_STORED`，並依 POSIX relative path 排序、固定 `create_system=3`，排除不同 runner 的 zlib、Path ordering 與 creator metadata byte drift。新增 CRLF／LF checkout 產生 byte-identical ZIP、順序／metadata 固定且 binary 原樣保留的回歸測試，並重建 Claude Code／Codex ZIP。
