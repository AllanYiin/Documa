...
- Action: Pick the route that matches the question shape:
  ...
  - Multi-document breadth question ("which documents mention X"): call `documa_search_collection` with `group_by_document=true` — each rollup carries an exact `hit_count`, the best snippet, and up to three read-ready `top_blocks`. Then narrow with `document_ids=[...]` (the `doc-` registry ids from the rollups) plus `per_document_limit` for depth inside the shortlisted documents.
...
- Token controls: set `max_chars` or `max_tokens` on `documa_read_block` (the tighter wins); set `max_response_tokens` on `documa_search_blocks` and `documa_search_collection` as a hard response ceiling; set `include_snippets=false` when only block ids are needed. Token budgets need a configured counter (tiktoken auto-detected, or `DOCUMA_TOKEN_COUNTER=anthropic:<model>` for Claude counting); on `TOKEN_COUNTER_UNAVAILABLE`, fall back to `max_chars`.
- Collection responses carry the same guidance surface as single-document search: follow `recommended_next` (a ready `documa_read_block` call chaining `read_ref`) and the response `hints` (any_term degradation, `offset=N` paging, group-mode suggestion when hits span many documents) before inventing a new strategy.
