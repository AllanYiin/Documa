...
2. For a structure/outline overview, call `documa_block_tree` with `max_depth=2-3, include_sketches=true`. For a source-preserving summary, call `documa_summarize`; local Rust LingXi selects exact clauses with block/page refs and no LLM call. Treat `top_k` as a soft limit and fall back explicitly to tree/search/read if the provider is unavailable. For facts, start `documa_search_blocks` with `limit=6, max_snippets_per_block=1`; use 2-4 discriminative lexical literals or quoted phrases in `query`, and only non-duplicative synonyms/spelling variants in `any_of`. Split multi-theme questions into one bounded search per theme instead of concatenating high-frequency terms.
