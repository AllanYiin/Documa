...
2. Route by question shape. Structure/overview: `documa_block_tree` with `max_depth=2-3, include_citations=false`, or `documa_list_blocks depth=1`, then descend with `parent_id` + `limit`/`offset`. Specific fact: `documa_search_blocks` with 2-4 precise terms, `verbosity=compact`, `limit<=5`, bilingual synonyms in `any_of`, quoted phrases for adjacency. Multi-document breadth ("which documents mention X"): `documa_search_collection` with `group_by_document=true`, then narrow with `document_ids=[...]` + `per_document_limit`; multi-document fact: `documa_search_collection` directly. `match_mode: "any_term"` in the response means precision degraded. Ingest/delete keep the collection index fresh automatically; `documa_index_collection` is the repair path when `documa_doctor` reports staleness. Treat search snippets as navigation only. Note: mailbox ingestion does not enter the collection — `documa_ingest` each `.eml`/`.msg` to make email searchable cross-document.
3. Follow the search response's `recommended_next` (block ids + `max_chars`) into `documa_read_block`; set `max_chars`/`max_tokens` from `recommended_read_chars` (token budgets need a configured counter — tiktoken auto-detected or `DOCUMA_TOKEN_COUNTER=anthropic:<model>`; on `TOKEN_COUNTER_UNAVAILABLE` use `max_chars`). Collection hits chain `read_ref.ir_path` (a `doc-` registry id) + `block_id` straight into `documa_read_block`.
