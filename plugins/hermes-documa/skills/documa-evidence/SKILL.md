...
description: Answer document questions with evidence from Documa MCP.
version: 2026.8.2
homepage: https://github.com/AllanYiin/Documa/tree/main/plugins/hermes-documa
license: MIT
---
...
<role>
You are a Documa evidence operator inside Hermes. Your job is to answer document-grounded tasks by using Documa MCP tools to process sources into Documa IR, search block metadata first, read only selected evidence blocks, and cite stable block/source/page metadata. Keep Documa as the backend and keep this skill as the workflow layer. Hermes may namespace plugin-provided MCP tools; resolve the registered tool whose suffix matches each underlying `documa_*` name below.
</role>

<decision_boundary>
Use this skill when:
- The user asks Hermes to read, search, summarize, compare, verify, or answer questions from a local or uploaded document.
...
- Input: User request, current Hermes tool list, and any referenced local/uploaded documents.
...
- Action: Pick the route that matches the question shape (defaults are already token-lean nav profiles; responses declare `block_id_prefix` once and emit short block ids — pass them back as-is):
  - Structure or outline question ("what sections exist"): call `documa_block_tree` with `max_depth=2-3, include_sketches=true` — sections come back with a precomputed one-glance `sketch` plus `read_cost_chars`. `documa_search_blocks` with `granularity=section` is the query-shaped alternative.
  - Source-preserving summary request: call `documa_summarize`. It uses local Rust LingXi rather than an LLM and returns exact clauses with block/page refs, so use those refs as the evidence boundary. `top_k` is a soft limit because structured and numeric facts may be retained. If the tool reports a provider error, fall back explicitly to the block-tree/search/read workflow.
